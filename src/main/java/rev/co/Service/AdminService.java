...
public class AdminService {
	...
	public static void addEmployee(int adminId) {
		...
		try (Connection con = DBUtil.getConnection()) {
			...
			PreparedStatement psCheckUser = con.prepareStatement("SELECT COUNT(*) FROM users WHERE employee_id = ?");
	...
	public static void viewEmployees(int adminId) {
		...
		try (Connection con = DBUtil.getConnection()) {
			...
			try (Statement st = con.createStatement(); ResultSet rs = st.executeQuery(sqlEmployees)) {

				System.out.println("\n========== EMPLOYEE LIST ==========");
				System.out.printf("%-10s %-15s %-30s %-15s %-10s%n", "ID", "Name", "Email", "Phone", "Status");
				System.out.println("---------------------------------------------------------------");

				boolean found = false;
				while (rs.next()) {
					found = true;
					System.out.printf("%-10d %-15s %-30s %-15s %-10s%n", rs.getInt("employee_id"), rs.getString("name"),
			...
			try (Statement st = con.createStatement(); ResultSet rs = st.executeQuery(sqlManagers)) {
	...
	public static void updateEmployeeStatus(int adminId) {
		...
		try (Connection con = DBUtil.getConnection(); PreparedStatement ps = con.prepareStatement(sql)) {
			...
			if (rows > 0) {
				...
				DBUtil.logAction(adminId, "Updated employee " + empId + " status to " + status);
	...
	public static void assignManager(int adminId) {
		...
		logger.info("Admin {} assigning manager {} to employee {}", adminId, managerId, empId);
		...
		try (Connection con = DBUtil.getConnection()) {
			...
			if (rows > 0) {
				DBUtil.logAction(adminId, "Assigned manager " + managerName + " to employee " + empId);

				System.out.println("✅ Manager " + managerName + " assigned successfully!");
	...
	public static void viewLeaveReports(int adminId) {
		...
		try (Connection con = DBUtil.getConnection()) {
			...
			String sql = "SELECT e.employee_id, e.name, lt.leave_name, " + "lr.start_date, lr.end_date, lr.status "
					+ "FROM leave_requests lr " + "JOIN employees e ON lr.employee_id = e.employee_id "
					+ "JOIN leave_types lt ON lr.leave_type_id = lt.leave_type_id " + "ORDER BY lr.start_date DESC";

			ResultSet rs = con.createStatement().executeQuery(sql);

			System.out.println("\n===== LEAVE REPORTS =====");
			System.out.printf("%-6s %-15s %-15s %-12s %-12s %-10s%n", "ID", "Employee", "Leave Type", "From", "To",
					"Status");
			System.out.println("---------------------------------------------------------------------");
			...
			while (rs.next()) {
				found = true;
				System.out.printf("%-6d %-15s %-15s %-12s %-12s %-10s%n", rs.getInt("employee_id"),
						rs.getString("name"), rs.getString("leave_name"), rs.getDate("start_date"),
	...
	public static void addHoliday(int adminId) {
		...
		try (Connection con = DBUtil.getConnection()) {
			...
			PreparedStatement check = con.prepareStatement("SELECT COUNT(*) FROM holidays WHERE holiday_date = ?");
	...
	public static void addDepartment(int adminId) {
		...
		try (Connection con = DBUtil.getConnection()) {
			...
			PreparedStatement check = con
					.prepareStatement("SELECT COUNT(*) FROM departments WHERE LOWER(department_name) = LOWER(?)");
			...
			seqStmt.execute("BEGIN " + "EXECUTE IMMEDIATE 'ALTER SEQUENCE department_seq INCREMENT BY 1'; "
					+ "EXECUTE IMMEDIATE 'SELECT MAX(department_id) + 1 INTO :nextval FROM departments'; "
					+ "EXECUTE IMMEDIATE 'ALTER SEQUENCE department_seq INCREMENT BY 1'; " + "END;");
	...
	public static void viewAllPerformanceReviews(int adminId) {
		...
		try (Connection con = DBUtil.getConnection()) {
			...
			String sql = "SELECT e.employee_id, e.name, pr.year, " + "pr.self_rating, pr.manager_rating, pr.status "
			...
			System.out.println("\n===== PERFORMANCE REVIEWS =====");
			System.out.printf("%-6s %-15s %-6s %-12s %-15s %-10s%n", "ID", "Name", "Year", "Self Rating",
			...
			while (rs.next()) {
				found = true;
				System.out.printf("%-6d %-15s %-6d %-12d %-15d %-10s%n", rs.getInt("employee_id"), rs.getString("name"),
						rs.getInt("year"), rs.getInt("self_rating"), rs.getInt("manager_rating"),
						rs.getString("status"));
			}
	...
	public static void viewAuditLogs(int adminId) {
		...
		try (Connection con = DBUtil.getConnection()) {
			...
			String sql = "SELECT user_id, action, log_time " + "FROM audit_logs " + "ORDER BY log_time DESC";

			ResultSet rs = con.createStatement().executeQuery(sql);

			boolean hasData = false;

			System.out.printf("%-10s %-45s %-25s%n", "USER ID", "ACTION", "LOG TIME");
			System.out.println("---------------------------------------------------------------");

			while (rs.next()) {
				hasData = true;
				System.out.printf("%-10d %-45s %-25s%n", rs.getInt("user_id"), rs.getString("action"),
