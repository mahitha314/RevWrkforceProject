	...
	public class ManagerService {
	        ...
	        try (Connection con = DBUtil.getConnection()) {
	            ...
	            while (rs.next()) {
	                ...
	                System.out.println(
	                    ...
	                    " | Type: " + rs.getString("leave_name") +
	                    " | From: " + rs.getDate("start_date") +
	    ...
	    public static void updateLeaveStatus(int managerId) {
	        ...
	        try (Connection con = DBUtil.getConnection()) {
	            ...
	            String sqlUpdate = "UPDATE leave_requests SET status = ? " +
	                               "WHERE leave_id = ? AND EXISTS " +
	            ...
	            if (status.equals("APPROVED")) {
	                String deductSql = "UPDATE leave_balance " +
	                ...
	                if (deducted == 0) {
	                    ...
	                    System.out.println("✅ Leave " + leaveId + " approved and " + leaveDays + " days deducted from balance.");
	                }
	            } else {
	    ...
	    public static void viewTeamLeaves(int managerId) {
	        try (Connection con = DBUtil.getConnection()) {
	            ...
	            String sql = "SELECT e.name, lr.start_date, lr.end_date, lr.status " +
	                         "FROM leave_requests lr " +
	                         "JOIN employees e ON lr.employee_id = e.employee_id " +
	                         "WHERE e.manager_id = ?";
	            ...
	            while (rs.next()) {
	                hasLeaves = true;
	                String leaveInfo = String.format("%s | %s to %s | %s",
	                        rs.getString("name"),
	    ...
	    public static void viewTeamGoals(int managerId) {
	        try (Connection con = DBUtil.getConnection()) {
	            ...
	            String sql = "SELECT e.name, g.description, g.deadline, g.priority, g.progress, g.status " +
	                         "FROM goals g " +
	            ...
	            while (rs.next()) {
	                hasGoals = true;
	                String goalInfo = String.format("%s | %s | %s | %s | %d%% | %s",
	                        rs.getString("name"),
	    ...
	    public static void viewTeamPerformance(int managerId) {
	        try (Connection con = DBUtil.getConnection()) {
	            ...
	            while (rs.next()) {
	                hasReviews = true;
	                String reviewInfo = String.format(
	                        "Employee: %s | Year: %d | Self: %d | Manager: %d | Status: %s",
	                        rs.getString("name"),
	                        rs.getInt("year"),
	    ...
	    public static void viewTeamAttendance(int managerId) {
	        try (Connection con = DBUtil.getConnection()) {
	            ...
	            String sql = "SELECT e.name, a.attendance_date, a.status " +
	                         "FROM attendance a " +
	    ...
	    public static void sendNotificationToTeam(int managerId) {
	        ...
	        try (Connection con = DBUtil.getConnection()) {
	            ...
	            String sql = "INSERT INTO notifications (notification_id, employee_id, message, type) " +
	                         "SELECT NOTIFICATIONS_SEQ.NEXTVAL, employee_id, ?, ? " +
	    ...
	    public static void viewAnnouncements() {
	        try (Connection con = DBUtil.getConnection()) {
	            ...
	            ResultSet rs = con.createStatement()
	                              .executeQuery(
	                                      "SELECT title, message, created_on FROM announcements ORDER BY created_on DESC");
	            ...
	            while (rs.next()) {
	                ...
	                String output = rs.getTimestamp("created_on") + " | " +
	                                rs.getString("title") + " | " +
